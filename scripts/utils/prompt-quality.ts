/**
 * [INPUT]: 依赖 twitterapi.io 候选字段、本地既有提示词指纹与 Muse Image 明确模型证据
...
export function scoreStoredTwitterPrompt(prompt: StoredTwitterPrompt): QualityResult {
  ...
  const hasPromptProvenance =
    /(?:tweet_text|reply_text|thread_reply|quoted_tweet|alt_text)_prompt|rewritten|normalized|editorial|expanded|specified/.test(
      promptSource
    );
  ...
  if (prompt.sourceMeta?.model_evidence !== "muse-image") {
    hardFailures.push("missing verified Muse Image model evidence");
  }
  ...
  } else if (/(rewritten|normalized|editorial|expanded|specified)/.test(promptSource)) {
    score += 15;
...
export function scoreTwitterCandidate(candidate: TwitterCandidate): QualityResult {
  ...
  const hasMuseImageEvidence = /\bmuse[-\s]*image\b/i.test(
    relevanceText
  );
  if (hasPromptSignal) {
    score += 30;
  ...
  if (!hasMuseImageEvidence) {
    reasons.push("missing explicit Muse Image evidence; manual review required");
